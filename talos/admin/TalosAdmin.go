...
func NewTalosAdminByConfig(talosClientConfig *client.TalosClientConfig,
	...
	socketTimeout := time.Duration(
		common.GALAXY_TALOS_CLIENT_ADMIN_TIMEOUT_MILLI_SECS_DEFAULT * time.Millisecond)
	talosClientFactory := client.NewTalosClientFactory(talosClientConfig, credential, socketTimeout)
...
func (a *TalosAdmin) CreateTopic(request *topic.CreateTopicRequest) (*topic.CreateTopicResponse, error) {
	if strings.HasPrefix(a.credential.GetSecretKeyId(), common.TALOS_CLOUD_AK_PREFIX) ||
		strings.HasPrefix(a.credential.GetSecretKeyId(), common.TALOS_GALAXY_AK_PREFIX) {
