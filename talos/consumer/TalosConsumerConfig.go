...
func NewTalosConsumerConfigByFilename(filename string) (*TalosConsumerConfig, error) {
	return NewTalosConsumerConfigByProperties(utils.LoadProperties(filename))
}

func NewTalosConsumerConfigByProperties(props *utils.Properties) (*TalosConsumerConfig, error) {
	talosConsumerConfig := initConsumerConfig(props)
	if err := talosConsumerConfig.CheckParameter(); err != nil {
		log.Error("Parameter's wrong : %s", err.Error())
		return nil, err
	}

	return talosConsumerConfig, nil
}
