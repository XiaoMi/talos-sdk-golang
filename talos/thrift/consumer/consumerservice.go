...
func NewConsumerServiceProcessor(handler ConsumerService) *ConsumerServiceProcessor {
	self21 := &ConsumerServiceProcessor{common.NewTalosBaseServiceProcessor(handler)}
	self21.AddToProcessorMap("lockPartition", &consumerServiceProcessorLockPartition{handler: handler})
	self21.AddToProcessorMap("lockWorker", &consumerServiceProcessorLockWorker{handler: handler})
	self21.AddToProcessorMap("unlockPartition", &consumerServiceProcessorUnlockPartition{handler: handler})
	self21.AddToProcessorMap("renew", &consumerServiceProcessorRenew{handler: handler})
	self21.AddToProcessorMap("updateOffset", &consumerServiceProcessorUpdateOffset{handler: handler})
	self21.AddToProcessorMap("queryOffset", &consumerServiceProcessorQueryOffset{handler: handler})
	self21.AddToProcessorMap("queryWorker", &consumerServiceProcessorQueryWorker{handler: handler})
	return self21
}
