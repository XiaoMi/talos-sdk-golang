...
func (s *MessageSerializerV2) Deserialize(header []byte, buf *bytes.Buffer) (*message.Message, error) {
	...
	if n != CREATE_TIMESTAMP_BYTES_V2 || err != nil {
		log4go.Error("read create timestamp error: %s", err.Error())
		return nil, err
	}
	...
	if n != MESSAGE_TYPE_BYTES_V2 || err != nil {
		log4go.Error("read message type error: %s", err.Error())
		return nil, err
	}
	...
	if n != SEQUENCE_NUMBER_LENGTH_BYTES_V2 || err != nil {
		log4go.Error("read sequence number length error: %s", err.Error())
		return nil, err
	}
	...
	if sequenceNumLen != 0 {
		...
		if n != int(sequenceNumLen) || err != nil {
			log4go.Error("read sequence number error: %s", err.Error())
			return nil, err
		}
	...
	if n != int(messageSize) || err != nil {
		log4go.Error("read message data error: %s", err.Error())
		return nil, err
	}
