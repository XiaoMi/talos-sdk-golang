...
func DoCompress(messageList []*message.Message,
	...
	for k, v := range messageList {
		err := serialization.NewMessageSerialization().SerializeMessage(v,
			messageSerializedBuffer, messageVersion)
		if err != nil {
			log.Error("compress MessageList failed" + err.Error())
			return nil, err
		}
		createTime += (v.GetCreateTimestamp() - createTime) / int64(k+1)
	}
	...
	if len(messageBlockData) > common.TALOS_MESSAGE_BLOCK_BYTES_MAXIMAL {
		err := fmt.Errorf("MessageBlock must be less than %d bytes, got bytes: %d",
			common.TALOS_MESSAGE_BLOCK_BYTES_MAXIMAL, len(messageBlockData))
		return nil, err
	}
...
func Decompress(messageBlockList []*message.MessageBlock,
	...
	for i := len(messageBlockList) - 1; i >= 0; i-- {
		...
		if err != nil {
			log.Error("decompress messageBlock error: %s ", err.Error())
			return nil, err
		}
