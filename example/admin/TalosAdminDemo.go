...
func (demo TalosAdminDemo) createTopic() *topic.CreateTopicResponse {
	...
	log.Infof("createTopic sucess: %s", topicName)
	return response
}
...
func (demo TalosAdminDemo) getTopicTalosResourceName() (
	*topic.TopicTalosResourceName, error) {
	...
	if err != nil {
		log.Errorf("getTopicTalosResourceName failed: %s", topicName)
		return nil, err
	}
...
func (demo TalosAdminDemo) deleteTopic(resourceName *topic.TopicTalosResourceName) error {
	...
	if err != nil {
		log.Errorf("Topic failed to delete: %s", resourceName)
		return err
	}
	log.Infof("Topic success to delete: %s", resourceName)
	return err
}
...
func (demo TalosAdminDemo) getTopicOffset(resourceName *topic.TopicTalosResourceName) error {
	...
	if err != nil {
		log.Errorf("Topic failed to getTopicOffset: %s", resourceName)
		return err
	}
	log.Infof("Topic success to getTopicOffset: %s", offsetInfoList)
	return err
}
...
func (demo TalosAdminDemo) listTopicsInfo() ([]*topic.Topic, error) {
	...
	for _, topicInfo := range topicInfoList {
		log.Infof("Topic info: %s", topicInfo)
	}
	return topicInfoList, err
}
